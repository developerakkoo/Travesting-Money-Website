// Travesting Exclusive Page JavaScript

document.addEventListener('DOMContentLoaded', function() {
    initializeTravestingExclusive();
});

function initializeTravestingExclusive() {
    initializeQA();
...
function initializeFormHandling() {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            ...
            const investmentExperience = this.querySelector('select[aria-label="Select investment experience"]').value;
            const preferredPlan = this.querySelector('select[aria-label="Select preferred plan"]').value;
            const message = this.querySelector('textarea').value;
            
            // Basic validation
            if (!name || !email || !phone || !investmentExperience || !preferredPlan || !message) {
                showNotification('Please fill in all fields', 'error');
                return;
            }
            
            // Show success message
            showNotification('Thank you for choosing Travesting Exclusive! We will contact you soon.', 'success');
...
function initializePremiumEffects() {
    ...
    // Add pricing button effects
    initializePricingButtons();
}

// Create sparkle animations
function createSparkleAnimations() {
    const heroSection = document.querySelector('.exclusive-hero');
    if (!heroSection) return;
    
    const sparklesContainer = heroSection.querySelector('.premium-elements');
...
function initializePremiumCardEffects() {
    const premiumCards = document.querySelectorAll('.feature-card.premium');
...
function initializeFeaturedCardStyling() {
    ...
    featuredCard.style.boxShadow = '0 20px 60px rgba(251, 191, 36, 0.15)';
...
function initializePricingButtons() {
    ...
    pricingButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            showNotification(`Thank you for choosing ${plan}! We will contact you soon for premium access.`, 'success');
...
// Add premium card shine effects
function initializePremiumCardShine() {
    const premiumCards = document.querySelectorAll('.feature-card.premium');
...
// Initialize premium card shine effects
document.addEventListener('DOMContentLoaded', function() {
    addShineAnimation();
    initializePremiumCardShine();
});

// Add value proposition card animations
function initializeValuePropositionAnimations() {
    ...
    premiumCard.style.transform = 'scale(0.8)';
    premiumCard.style.transition = 'opacity 0.8s ease, transform 0.8s ease';
    observer.observe(premiumCard);
}

// Initialize value proposition animations
document.addEventListener('DOMContentLoaded', function() {
    initializeValuePropositionAnimations();
});

// Add premium button hover effects
function initializePremiumButtonEffects() {
    const premiumButtons = document.querySelectorAll('.btn-premium, .pricing-btn.premium');
